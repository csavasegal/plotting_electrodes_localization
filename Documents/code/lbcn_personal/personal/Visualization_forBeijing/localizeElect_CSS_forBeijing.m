function [subjVar_final] = localizeElect_CSS_forBeijing(subjVar,dirs,chanInfo)
...
for i = 1:length(elinfo.Destrieux)
    ...
    if ~any(rows)   % if empty channel
        ...
        elinfo.Yeo7_ind(i) = cell_Yeo7info(rows==1,1); %1, Clara change
...
for i = 1:length(elinfo.Destrieux)
    ...
    if ~any(rows)   % if empty channel
        ...
        elinfo.Yeo17_ind(i) = cell_Yeo17info(rows==1,1); %1, Clara change
...
for i = 1:length(subjVar.labels)
    if contains(subjVar.labels{i},'empty','IgnoreCase',true)  % if empty channel
        ...
        if strcmp(subjVar.sbj_name, 'C17_21') || strcmp(subjVar.sbj_name, 'C18_29') %all left
            ...
            rows = A & B;
            subjVar_final.elinfo(i,:) = elinfo(rows==1,:);
        elseif strcmp(subjVar.sbj_name, 'C18_35')
      
            
            %A = any(strcmp(cell_elinfo, subjVar.labels{i}), 2) ;
            hemi_temp = string(chanInfo.Hem(((any(strcmp(subjVar.labels{i}, chanInfo.Namenew),2))==1),:));
            %B = any(strcmp(cell_elinfo, chanInfo.Hem{i}), 2);
            B = any(strcmp(cell_elinfo,hemi_temp),2);
            num_temp = erase(subjVar.labels{i},"'");
            A = any(strcmp(cell_elinfo, num_temp), 2) ;
            
            rows = A & B;
            subjVar_final.elinfo(i,:) = elinfo(rows==1,:);
            
            %any(strcmp(subjVar.labels{i}, chanInfo.Namenew),2)
