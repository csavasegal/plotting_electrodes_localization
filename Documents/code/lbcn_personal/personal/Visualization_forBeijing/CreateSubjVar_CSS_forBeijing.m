function [subjVar, subjVar_created]  = CreateSubjVar_CSS_forBeijing(sbj_name, comp_root, server_root, code_root)
...
[fs_iEEG, fs_Pdio, data_format] = GetFSdataFormat(sbj_name, 'China');
dirs = InitializeDirs('Memoria', sbj_name, comp_root, server_root, code_root); % 'Pedro_NeuroSpin2T'
...
if strcmp(sbj_name, 'C18_39') || strcmp(sbj_name, 'C18_35')  %|| strcmp(sbj_name, 'C18_45')  strcmp(sbj_name, 'C18_29') ||
    fs_chan_names_temp = chanInfo.elecNames
    for i=1:length(chanInfo.elecNames)
        if strcmp(fs_chan_names_temp{i}(1),'L')
            fs_chan_names_new{i,1} = [chanInfo.Name{i}(1),char(39),chanInfo.Name{i}(2:end)]
...
if strcmp(sbj_name, 'C18_35')
    chanInfo.Namenew = fs_chan_names_new;
end
...
if ~exist('mismatch_labels')
    ...
    %subjVar = localizeElect_test(subjVar,dirs,chanInfo);
