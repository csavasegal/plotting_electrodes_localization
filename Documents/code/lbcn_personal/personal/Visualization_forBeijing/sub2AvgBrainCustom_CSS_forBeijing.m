function [avgCoords, elecInfo_table, elecCoord, avgVids, subVids]=sub2AvgBrainCustom_CSS_forBeijing(cfg, dirs, sbj_name, fsDir_local)
%modified by CSS to account for Beijing system of naming data.
%Changes made on 218
...
for hemLoop=1:2,
    ...
    if ~isempty(hemElecIds)
        ...
        %CSS change here
        for b=1:length(elecNames)
            elecNames(b) = append(elecInfo_table.Hem(b),elecInfo_table.Name(b))
            elecNamesDepths(b) = append(elecInfo_table.Hem(b),elecInfo_table.Name(b))
        end 
...
elecInfo_table.elecNames = elecNames;
