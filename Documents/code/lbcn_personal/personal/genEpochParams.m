...
switch project_name
    ...
    case {'Scrambled', 'Logo', 'VTCLoc', 'SevenHeaven', 'ReadNumWord', 'AllCateg'}
        epoch_params.bef_time = -0.250;
        epoch_params.aft_time = 2;
        epoch_params.blc.run = true; % or false
        epoch_params.blc.win = [-.250 0];
    case 'race_encoding'
        strcmp(locktype, 'stim')
        epoch_params.bef_time = -.2;
        %             epoch_params.aft_time = 5;
        epoch_params.aft_time = 2.5;
        %         elseif strcmp(locktype, 'resp')
        %             epoch_params.bef_time = -3;
        %             epoch_params.aft_time = 1;
        %         end
    ...
    case 'race_faces'
        ...
        %         end
        epoch_params.blc.run = true; % or false
        epoch_params.blc.win = [-2 0]; %should be the ITI
    case 'emotional_faces'
        strcmp(locktype, 'stim')
        
        %if plotting all three stim onsets
        %         epoch_params.bef_time = -.2;%-.2;
        %         epoch_params.aft_time = 6.5%1;
